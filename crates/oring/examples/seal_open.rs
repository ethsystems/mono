...
fn main() {
    ...
    let recipient_sk = StaticSecret::random_from_rng(&mut rng);
    ...
    let envelope =
        seal::<X25519, WalletDomain>(&recipient_pk, &note, aad, &mut rng).unwrap();
    println!("sealed {} bytes", envelope.as_bytes().len());

    let opened =
        open::<X25519, WalletDomain, _>(&recipient_sk, &recipient_pk, &envelope, aad)
            .unwrap();
    ...
    let mut scanner = Scanner::<X25519, WalletDomain>::new(recipient_sk, &recipient_pk);
